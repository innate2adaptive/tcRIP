...
parser = ArgumentParser(description="Decombinator ShortReads_nopairs")
parser.add_argument("-i", dest="filename", required=True)
parser.add_argument("-o", dest="output1", required=True)
...
def setup(vregions_file, jregions_file):
    ...
    for v in v_genes_cut:
        all_v_substrings.append([v[i:i+n] for n in range(start, len(v)+1) for i in range(len(v)-(n-1))])
    ...
    for j in j_genes_cut:
        all_j_substrings.append([j[i:i+n] for n in range(start, len(j)+1) for i in range(len(j)-(n-1))])
...
def analyse_file(infile, outpath, fileid, v_keyword_tries, j_keyword_tries, v_genes, j_genes, parameters):
    ...
    outfile = output + '/' + output + '_beta.txt'
...
def assign_by_longest_substring(substrings_found_by_gene, parameters):
    ...
    for matches in substrings_found_by_gene:
        lengths = [len(x[0]) for x in matches]
        if lengths:
            longest_substring_by_gene.append(max(lengths))
        else:
            longest_substring_by_gene.append(0)
...
if __name__ == '__main__':
    ...
    vfile = 'D:/Dropbox/github/Decombinator_v2/human_TRBV_region.fasta'
    jfile = 'D:/Dropbox/github/Decombinator_v2/human_TRBJ_region.fasta'

    v_key, j_key, v_regions, j_regions = setup(vfile, jfile)

#    infile = 'testseqs.fastq'
#    infile = 'SRR4417807_veryshort.fastq'
#    infile = 'SRR4417808_1.fastq'
#    outpath = 'D:/Decombinator_v2/results_SRR4417807/'
    fileid = infile
