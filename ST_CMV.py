...
if inLine:
    ...
    with open("data/CMV/cmv.txt",'r') as infile:
        cmv_beta=[]
        for line in infile:
            cmv_beta.append(line[:-1])
...
cmv_beta=dp.filtr(cmv_beta, 13)
#ebv_beta=dp.filtr(ebv_beta, 13)
...
y=np.concatenate((y,np.zeros((len(cmv_work)))),0)
X=np.concatenate((X,cmv_work),0)
